                                                  ...
                                                  const char *device_address)
{
    ...
    if (AudioSystem::isOutputDevice(device)) {
        ...
        case AudioSystem::DEVICE_STATE_UNAVAILABLE: {
            ...
            mAvailableOutputDevices &= ~device;
	     LOGV("yyd- avaliableOutputDevices= %d",mAvailableOutputDevices);
...
#endif
            {
                if (AudioSystem::isBluetoothScoDevice(device)) {
                    ...
                    LOGV("setDeviceConnectionState() Sco device");
                    if (wasUsedforPhone &&
                        mPhoneState == AudioSystem::MODE_IN_CALL) {
                    ...
                    // 4: the strategy DTMF is active on the hardware output:
                    //      use device for strategy DTMF
                    LOGV("setDeviceConnectionState() headset device");
                    if (wasUsedforPhone &&
                        (mPhoneState == AudioSystem::MODE_IN_CALL ||
                         mOutputs.valueFor(mHardwareOutput)->isUsedByStrategy(STRATEGY_PHONE))) {
                        newDevice = getDeviceForStrategy(STRATEGY_PHONE);
	     LOGV("yyd- wasUsedforPhone, newDevice= %d",newDevice);
                    } else if (wasUsedForSonification &&
                               mOutputs.valueFor(mHardwareOutput)->isUsedByStrategy(STRATEGY_SONIFICATION)){
                        newDevice = getDeviceForStrategy(STRATEGY_SONIFICATION);
	     LOGV("yyd- wasUsedForSonification, newDevice= %d",newDevice);
                    } else if (wasUsedForMedia &&
                               mOutputs.valueFor(mHardwareOutput)->isUsedByStrategy(STRATEGY_MEDIA)){
                        newDevice = getDeviceForStrategy(STRATEGY_MEDIA);
	     LOGV("yyd- wasUsedForMedia, newDevice= %d",newDevice);
                    } else if (wasUsedforDtmf &&
                               mOutputs.valueFor(mHardwareOutput)->isUsedByStrategy(STRATEGY_DTMF)){
                        newDevice = getDeviceForStrategy(STRATEGY_DTMF);
	     LOGV("yyd- wasUsedforDtmf, newDevice= %d",newDevice);
                    }
                } else if (device == AudioSystem::DEVICE_OUT_TTY) {
                    LOGV("setDeviceConnectionState() tty device");
                    if (wasUsedforPhone &&
        ...
        if (device == AudioSystem::DEVICE_OUT_WIRED_HEADSET) {
	     ...
	     LOGV("yyd- headset, Device in-out = %d",device);
        } else if (device == AudioSystem::DEVICE_OUT_BLUETOOTH_SCO ||
                   device == AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_HEADSET ||
                   device == AudioSystem::DEVICE_OUT_BLUETOOTH_SCO_CARKIT) {
            device = AudioSystem::DEVICE_IN_BLUETOOTH_SCO_HEADSET;
	     LOGV("yyd- bluetooth, Device in-out = %d",device);
        } else {
	     LOGV("yyd- others, Device in-out ");
            return NO_ERROR;
        }
    }
...
: mPhoneState(AudioSystem::MODE_NORMAL), mRingerMode(0), mMusicStopTime(0)
{
	...
	LOGV("yyd- first setOutputDevice ~~~");
       setOutputDevice(mHardwareOutput, (uint32_t)AudioSystem::DEVICE_OUT_SPEAKER, true);// yyd 100511, org
    }
...
//#ifdef WITH_A2DP // yyd- 100719
    if (output == mHardwareOutput) {
        ...
        // in this case
            LOGV("yyd- setOutputDevice() output == mA2dpOutput");
        device &= ~AudioSystem::DEVICE_OUT_SPEAKER;
    }
//#endif
...
//		device = 2;
    ...
    if (device == (AudioSystem::DEVICE_OUT_SPEAKER | AudioSystem::DEVICE_OUT_WIRED_HEADSET) ||
        ...
        LOGV("yyd- both speaker and headser, sleep\n");
    ...
    if (oldDevice == (AudioSystem::DEVICE_OUT_SPEAKER | AudioSystem::DEVICE_OUT_WIRED_HEADSET) ||
        ...
        LOGV("yyd- change from both speaker and headser, unmute\n");
    }
#if 1 //yyd- 100719 route for audio
  {
#if 1
#if 0
   if(device != 0)
  {
	...
	{
		 LOGV("yyd- speaker~~~~\n");
		system("alsa_amixer  cset name='Speaker Function' 1");
	}
	else
	{
		 LOGV("yyd- no speaker~~~~,device = %d \n",device);
		system("alsa_amixer  cset name='Speaker Function' 0");
	}
    }
#endif
	...
	{
		 LOGV("yyd- speaker ~~~~\n");
		system("alsa_amixer cset name='HP/SPK Route Sel',index=0 1");
	}
	else if (device == AudioSystem::DEVICE_OUT_EARPIECE)
	{
		 LOGV("yyd-  earpiece ~~~~\n");
		system("alsa_amixer cset name='HP/SPK Route Sel',index=0 0");
	}
	
	else if (device == AudioSystem::DEVICE_OUT_BLUETOOTH_SCO)
	{
		 LOGV("yyd- BT_SCO ~~~~\n");
		system("alsa_amixer cset name='BT Route Sel',index=0 1");			
	}
	else if (device == AudioSystem::DEVICE_OUT_BLUETOOTH_A2DP)
	{
		 LOGV("yyd- BT_A2DP ~~~~\n");
		system("alsa_amixer cset name='BT Route Sel',index=0 1");			
	}
  	...
  	if (mPhoneState == AudioSystem::MODE_IN_CALL) 
  	{
  		LOGD("MODE_IN_CALL");
		system("alsa_amixer  cset name='Speaker Function' 1");
//  		system("alsa_amixer cset name='Android Mode Set',index=0 2");	// MODE_IN_CALL
		if (device == AudioSystem::DEVICE_OUT_SPEAKER)			
			system("alsa_amixer cset name='HP/SPK Route Sel',index=0 1");
		else if (device == AudioSystem::DEVICE_OUT_EARPIECE)
			system("alsa_amixer cset name='HP/SPK Route Sel',index=0 0");
	...
	else if (mPhoneState == AudioSystem::MODE_RINGTONE)
	{
		LOGD("MODE_IN_RINGTONE");
//		system("alsa_amixer cset name='Android Mode Set',index=0 1");	// MODE_RINGTONE
		system("alsa_amixer  cset name='Speaker Function' 1");
	}
	else if (mPhoneState == AudioSystem::MODE_NORMAL)
	{
		LOGD("MODE_IN_NORMAL");
//		system("alsa_amixer cset name='Android Mode Set',index=0 0");			// MODE_NORMAL
		system("alsa_amixer  cset name='Speaker Function' 1");
	}
	else
	{
		LOGD("MODE_IN_NORMAL default");
		//system("alsa_amixer cset name='Android Mode Set',index=0 0");			// MODE_NORMAL
		system("alsa_amixer  cset name='Speaker Function' 1");
	}
#endif
